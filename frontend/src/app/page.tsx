  ...
  const handleChat = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/query`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ question: userMsg, top_k: 5, model: "gemini-1.5-flash-8b" }),
      });
      const data = await res.json();
      if (res.ok) {
        setChatHistory((prev) => [
                      ...
                      <Card key={idx} className="bg-sidebar border-border hover:border-primary/50 rounded-xl shadow-none transition-none">
                        <CardHeader className="py-3 px-4 border-b border-border flex flex-row items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium">{result.metadata?.source || "Unknown Document"}</span>
                          </div>
                          <Badge variant="outline" className="text-xs bg-background border-border rounded-sm font-normal">
                            Score: <span className="text-primary font-medium ml-1">{result.score ? result.score.toFixed(4) : "N/A"}</span>
                          </Badge>
                        </CardHeader>
